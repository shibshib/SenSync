...
#define MINIMUM_SAMPLE_LENGTH 35
...
void fixFileOutput(std::string portName) {
	std::ifstream inputFile(dataPath + "raw_output_" + portName + ".csv");
	std::ofstream cfile;
	std::string filenum = "1";

	cfile.open(dataPath + "Accel_" + portName + ".csv", std::fstream::in | std::fstream::out | std::fstream::app);
	std::string line;

	while (std::getline(inputFile, line)) {
		if (strlen(line.c_str()) > MINIMUM_SAMPLE_LENGTH) {
			// erase final comma
			line.pop_back();
...
BOOL WINAPI ConsoleHandler(DWORD CEvent) {
	...
	case CTRL_CLOSE_EVENT:
		...
		fixFileOutput(COM9String);
		fixFileOutput(COM12String);
